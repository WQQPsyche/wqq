...
@interface DotaPickBattleController ()
{
...
@end
@implementation DotaPickBattleController
...
- (void)randChooseEmenySqquardronHeros{
    ...
    DotaPickHeroController *heroController = [[DotaPickHeroController alloc] init];
    heroController.delegate = self;
    
    for (int i = 1; i<=3; i++) {
        heroController.hero_Type = HERO_ARRAY[i-1];
        heroController.path = [WQQTools plistPathWithHeroType:HERO_ARRAY[i-1]];
        [heroController chooseEnemySqquardronHeros];
...
- (void)pushHeroViewControllerWithInNum:(int)inNum{
    ...
    DotaPickHeroController *heroScene = [[DotaPickHeroController alloc] init];
    //入栈
    [self.sceneManager pushController:heroScene];
    
    heroScene.hero_Type = HERO_ARRAY[inNum-1];
    heroScene.path = [WQQTools plistPathWithHeroType:HERO_ARRAY[inNum-1]];
    heroScene.delegate = self;
    [heroScene didLoadScene];
}
